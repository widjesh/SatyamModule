...
export class ContactsService implements Resolve<any> {
  ...
  getContacts(): Promise<any> {
    return new Promise((resolve, reject) => {
      this._httpClient
        .get("http://172.19.142.76:3000/customers/")
        .subscribe((response: any) => {
          ...
          if (this.searchText && this.searchText !== "") {
            this.customers = FuseUtils.filterArrayByString(
              this.customers,
              this.searchText
            );
  ...
  updateContact(customer): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      this._httpClient
        .patch(
          "http://172.19.142.76:3000/customers/update/" +
            customerToSave.contact.email,
          { ...customerToSave }
        )
        .subscribe(response => {
          this.getContacts();
  ...
  updateUserData(userData): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      this._httpClient
        .post("http://172.19.142.76:3000/customers/" + this.user.email, {
          ...userData
        })
        .subscribe(response => {
          this.getUserData();
